...
do
	...
	BattleSection:Toggle({
		...
		Callback = function(Value)
			...
			if Value == true then
				connections["AutoClick"] = game.RunService.Heartbeat:Connect(function()
					task.wait()
					local args = {
						buffer.fromstring("\v"),
					}
	...
	local BattlesList = {}
	for _, v in workspace.Worlds.Targets.Server:GetChildren() do
		if
			v:FindFirstChild("Units_Displays")
			and not table.find(BattlesList, v.Units_Displays.Names.Text)
			and not string.find(v.Name, "Raids")
		then
			table.insert(BattlesList, v.Units_Displays.Names.Text)
		end
	end
	workspace.Worlds.Targets.Server.ChildAdded:Connect(function(v)
		task.wait(1)
		if
			v:FindFirstChild("Units_Displays")
			and not table.find(BattlesList, v.Units_Displays.Names.Text)
		...
		then
			table.insert(BattlesList, v.Units_Displays.Names.Text)
			BattleSection.Elements[2]:Refresh(BattlesList)
		end
	end)
	...
	BattleSection:Toggle({
		...
		Callback = function(Value)
			...
			if Value == true then
				local target = ""
				connections["AutoFarm"] = game.RunService.Heartbeat:Connect(function()
					if selectedEnemy == "" then
						return
					end
					if
						target ~= ""
						and workspace.Worlds.Targets.Server:FindFirstChild(target)
						and plr.TargetAt.Value == workspace.Worlds.Targets.Server[target]
					then
						return
					end
					for _, v in workspace.Worlds.Targets.Server:GetChildren() do
						local distance = (plr.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude
						if distance < 350 and string.find(v.Units_Displays.Names.Text, selectedEnemy) then
							target = v.Name
							break
						end
					end
					requestBattle(target)
					task.wait(1)
					repeat
						task.wait()
					until not workspace.Worlds.Targets.Server:FindFirstChild(target)
						or plr.TargetAt.Value ~= workspace.Worlds.Targets.Server[target]
					print("Target found: ", workspace.Worlds.Targets.Server:FindFirstChild(target))
					print("atTarget: ", plr.TargetAt.Value == workspace.Worlds.Targets.Server[target])
				end)
			elseif Value == false then
	...
	local SummonsList = {}
	for _, v in workspace.Summoners:GetChildren() do
		if not table.find(SummonsList, v.Name) then
			table.insert(SummonsList, v.Name)
		end
	end
	workspace.Summoners.ChildAdded:Connect(function(v)
		if not table.find(SummonsList, v.Name) then
			table.insert(SummonsList, v.Name)
			BattleSection.Elements[4]:Refresh(SummonsList)
		end
	end)
	BattleSection:Listbox({
		Flag = "SummonsList",
	...
	BattleSection:Toggle({
		...
		Callback = function(Value)
			...
			if Value == true then
				connections["AutoSummon"] = game.RunService.Heartbeat:Connect(function()
					if selectedSummon == "" then
						return
					end
					summon(selectedSummon)
				end)
			elseif Value == false then
				if connections["AutoSummon"] then
					connections["AutoSummon"]:Disconnect()
				end
				return
			end
		end,
...
do
	local QuestSection = BattlesPage:Section({
		Name = "Quests",
	...
	local selectedQuest = ""
	local QuestsList = {}
	for _, v in RS.Players_Data[plr.Name].Quest:GetChildren() do
		if not table.find(QuestsList, v.Name .. " - " .. v.Display.Value) then
			table.insert(QuestsList, v.Name .. " - " .. v.Display.Value)
		end
	end
	RS.Players_Data[plr.Name].Quest.ChildAdded:Connect(function(v)
		if not table.find(QuestsList, v.Name .. " - " .. v.Display.Value) then
			table.insert(QuestsList, v.Name .. " - " .. v.Display.Value)
			QuestSection.Elements[1]:Refresh(QuestsList)
		end
	end)

	QuestSection:Listbox({
		Flag = "QuestsList",
		Default = "",
		Size = 100,
		Items = QuestsList,
	...
	QuestSection:Toggle({
		...
		Default = false,
		Callback = function(Value)
			getgenv().AutoCompleteQuest = Value
			if Value == true then
				connections["AutoCompleteQuest"] = task.spawn(function()
					while getgenv().AutoCompleteQuest do
						if selectedQuest == "" or selectedQuest == nil then
							return
						end
						local quest =
							RS.Players_Data[plr.Name].Quest:FindFirstChild(string.split(selectedQuest, " - ")[1])
						if quest.Finished.Value == true then
							QuestSection.Elements[2]:Set(false)
							Library:Notification({
								Title = "Quest Complete! ",
								Description = selectedQuest,
								Duration = 5,
								Icon = "73789337996373",
							})
							return
						...
						for _, questGroup in quest.Defeat_Groups:GetChildren() do
							local defeatNames = questGroup.Defeat_Names.Value
							local Event = questGroup.Event.Value
							local Maximum = questGroup.Maximum.Value
							local Numbers = questGroup.Numbers.Value

							if Maximum == Numbers then
								continue
							end

							local target = ""
							for i = 1, Maximum - Numbers do
								if Event == "Defeat" then
									repeat
										for __, enemies in workspace.Worlds.Targets.Server:GetChildren() do
											local distance = (
												plr.Character.HumanoidRootPart.Position
												- enemies.HumanoidRootPart.Position
											).Magnitude
											if
												distance < 350
												and (
													string.find(enemies.Units_Displays.Names.Text, defeatNames)
													or string.find(enemies.Humanoid.DisplayName, defeatNames)
												)
											then
												target = enemies.Name
												break
									...
									print("Requesting battle against: " .. target)
									requestBattle(target)
									repeat
										task.wait()
									until not workspace.Worlds.Targets.Server:FindFirstChild(target)
								...
								if Event == "Raids" then
									if defeatNames == "-" then
										for __, pods in workspace.Raids_Entering:GetChildren() do
											if pods.Holders.Value == nil or pods.Holders.Value == "" then
												plr.Character.HumanoidRootPart.CFrame = pods.Centers.CFrame
											end
										end
										...
										target = {
											RS.Shared.Parties[plr.Name],
											"Worlds_Namex Planet",
											"Difficulty_Easy",
											"Namex Planet",
											"Easy",
										}
										Remotes.Gameplays.RaidsLobbies:FireServer(unpack({ target[1], target[2] }))
										Remotes.Gameplays.RaidsLobbies:FireServer(unpack({ target[1], target[3] }))

										Remotes.Systems.RaidsEvent:FireServer(unpack({ target[4], target[5] }))
										Remotes.Gameplays.RaidsLobbies:FireServer(unpack({ target[1], [3] = true }))
										Remotes.Misc.Parties:FireServer(unpack({ "Disabled" }))
										...
										for __, enemies in workspace.Worlds.Targets.Server:GetChildren() do
											...
											if
												string.find(enemies.Name, "Raids")
												and enemies.Whitelist:findFirstChild(plr.Name)
											then
												table.insert(targets, enemies.Name)
											end
										end

										for inedx, currentTarget in targets do
											print("Requesting battle against: " .. currentTarget)
											requestBattle(currentTarget)
											repeat
												task.wait()
											until not workspace.Worlds.Targets.Server:FindFirstChild(currentTarget)
										end
									end
								end
							end
							...
							Remotes.Systems.QuestEvent:FireServer(unpack(args))
						end
			...
			elseif Value == false then
				if connections["AutoCompleteQuest"] then
					connections["AutoCompleteQuest"] = nil
				end
				return
			end
		end,
...
function requestBattle(Target)
	local args = {
		Target,
		"Mouse",
	}
	Remotes.Gameplays.Request:FireServer(unpack(args))
end

function summon(Target)
	local args = {
		workspace.Summoners[Target],
		"Multi",
	}
	Remotes.Summoners.RemoteEvent:FireServer(unpack(args))
end
