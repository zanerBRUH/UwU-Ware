...
do
	...
	do
			...
			function Colorpicker:SetOpen(Bool)
				...
				for Index, Value in Descendants do
					...
					if not Value.ClassName:find("UI") then
						Value.ZIndex = (Colorpicker.IsOpen and Data.Section.IsSettings and 9)
							or (Colorpicker.IsOpen and not Data.Section.IsSettings and 3)
							or 1
					end
			...
			Library:Connect(UserInputService.InputBegan, function(Input)
				...
				then
					...
					if
						...
						or Library:IsMouseOverFrame(Items["PaletteIcon"]) and not Data.Section.IsSettings
					then
						return
					end
		...
		Library.Notification = function(self, Data)
			...
			do
				...
				Items["Accent"] = Instances:Create("Frame", {
					...
					Position = UDim2New(
						...
						0,
						0,
						Items["Description"].Instance.AbsoluteSize.Y + Items["Title"].Instance.AbsoluteSize.Y + 12
					),
					BorderColor3 = FromRGB(0, 0, 0),
			...
			Library:Thread(function()
				...
				Items["Accent"]:Tween(
					TweenInfo.new(Data.Duration, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut),
		...
		Library.Window = function(self, Data)
			...
			do
				...
				function Window:SetTransparency()
					...
					if IsMobile then
						Items["FloatingButton"].Instance.BackgroundTransparency =
							Library.Flags["BackgroundTransparency"]
					end
				...
				do
					...
					SettingsItems["CloseButton"]:OnHover(function()
						SettingsItems["Accent"]:Tween(
							TweenInfo.new(Library.Tween.Time + 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
							{ Size = UDim2New(1, 0, 1, 0), BackgroundTransparency = 0 }
						)
					end)
					...
					function Settings:SetOpen(Bool)
						...
						if Settings.IsOpen then
							...
							RenderStepped = RunService.RenderStepped:Connect(function()
								SettingsItems["Settings"].Instance.Position = UDim2New(
									0,
									Items["SettingsIcon"].Instance.AbsolutePosition.X,
									0,
									Items["SettingsIcon"].Instance.AbsolutePosition.Y
										+ Items["SettingsButton"].Instance.AbsoluteSize.Y
										+ 108
								)
						...
						NewTween.Tween.Completed:Connect(function()
							...
							task.wait(0.2)
							SettingsItems["Settings"].Instance.Parent = not Settings.IsOpen
									and Library.UnusedHolder.Instance
								or Library.Holder.Instance
						end)
					end
			...
			do
				...
				Items.Gradient = Instances:Create("UIGradient", {
					...
					Transparency = NumSequence({
						NumSequenceKeypoint(0, 0.41874998807907104),
						NumSequenceKeypoint(0.445, 0.78125),
						NumSequenceKeypoint(0.751, 0.9375),
		...
		Library.Sections.Toggle = function(self, Data)
			...
			do
				...
				Items["Toggle"]:OnHover(function()
					Items["Indicator"]:Tween(
						...
						{ Size = UDim2New(0, 21, 0, 21), Position = UDim2New(0, -3, 0, -3) }
					)
				end)
				...
				Items["Toggle"]:OnHoverLeave(function()
					Items["Indicator"]:Tween(
						TweenInfo.new(0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
						{ Size = UDim2New(0, 18, 0, 18), Position = UDim2New(0, 0, 0, 0) }
					)
				end)
			end
			...
			function Toggle:Set(Value)
				...
				if Toggle.Value then
					Items["Accent"]:Tween(
						TweenInfo.new(Library.Tween.Time + 0.1, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
						{ BackgroundTransparency = 0, Size = UDim2New(1, 0, 1, 0) }
					)
					...
					Items["Accent"]:Tween(
						TweenInfo.new(Library.Tween.Time + 0.05, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
			...
			function Toggle:Settings(Size)
				...
				do
					...
					SettingsItem["Gradient"]:AddToTheme({
						Color = function()
							return RGBSequence({
								RGBSequenceKeypoint(0, Library.Theme.Accent),
								RGBSequenceKeypoint(1, Library.Theme.AccentGradient),
				...
				function Settings:SetOpen(Bool)
					...
					if Settings.IsOpen then
						...
						RenderStepped = RunService.RenderStepped:Connect(function()
							SettingsItem["Settings"].Instance.Position = UDim2New(
								0,
								Items["Toggle"].Instance.AbsolutePosition.X
									+ Items["Toggle"].Instance.AbsoluteSize.X / 1.9
									+ 15,
								0,
								Items["Toggle"].Instance.AbsolutePosition.Y
									+ Items["Toggle"].Instance.AbsoluteSize.Y
									+ Size / 1.9
							)
					...
					NewTween.Tween.Completed:Connect(function()
						...
						task.wait(0.2)
						SettingsItem["Settings"].Instance.Parent = not Settings.IsOpen and Library.UnusedHolder.Instance
							or Library.Holder.Instance
					end)
				end
				...
				if Dropdown.IsOpen then
					...
					RenderStepped = RunService.RenderStepped:Connect(function()
						...
						Items["OptionHolder"].Instance.Size =
							UDim2New(0, Items["RealDropdown"].Instance.AbsoluteSize.X, 0, Dropdown.OptionHolderSize)
					end)
			...
			function Label:RefreshPosition(Bool)
				if Bool then
					...
					if Items["SubElements"] then
						Items["SubElements"]:Tween(
							TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
							{ Position = UDim2New(0, 0, 0, 30) }
						)
						Tween:Create(
							Items["Label"].Instance:FindFirstChild("nig"),
							TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
							{ Position = UDim2New(1, -16, 1, -6) },
							true
						)
					...
					if Items["SubElements"] then
						Items["SubElements"]:Tween(
							TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
							{ Position = UDim2New(0, 30, 0, 30) }
						)
						Tween:Create(
							Items["Label"].Instance:FindFirstChild("nig"),
							TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
							{ Position = UDim2New(1, 30, 1, -6) },
							true
						)
					end
				end
			...
			function Keybind:SetMode(Mode) -- hard coded
				if Mode == "Toggle" then
					Items["Background"]:Tween(
						...
						{ Position = UDim2New(0, 0, 0, 0), Size = UDim2New(0.35, 0, 1, 0) }
					)
					Items["Toggle"]:ChangeItemTheme({
						TextColor3 = function()
				...
				elseif Mode == "Hold" then
					Items["Background"]:Tween(
						TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
						{ Position = UDim2New(0.35, 0, 0, 0), Size = UDim2New(0.35, 0, 1, 0) }
					)
				...
				elseif Mode == "Always" then
					Items["Background"]:Tween(
						TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
						{ Position = UDim2New(0.7, 0, 0, 0), Size = UDim2New(0.3, 0, 1, 0) }
					)
			...
			Library:Connect(UserInputService.InputBegan, function(Input)
				...
				if Input.UserInputType == Enum.UserInputType.MouseButton1 then
					...
					if
						...
						or Library:IsMouseOverFrame(Items["OptionHolder"])
					then
						return
					end
				...
				function OptionData:Search(Bool)
					Library:Thread(function()
						if Bool then
							...
							OptionText:Tween(
								TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
								{ TextTransparency = OptionData.Selected and 0 or 0.3 }
							)
							task.wait(0.08)
							OptionButton:Tween(
								TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
	...
	Library.CreateSettingsPage = function(self, Window, KeybindList)
		...
		local MiscSection = Page:Section({
			Name = "Misc",
		...
		do
			...
			MiscSection:Toggle({
				...
				Callback = function(Value)
					if Value == true then
						connections["Anti-Idle"] = plr.Idled:Connect(function()
							VirtualUser:CaptureController()
							VirtualUser:ClickButton2(Vector2.new())
						end)
					else
						if connections["Anti-Idle"] then
							connections["Anti-Idle"]:Disconnect()
						end
					end
				end,
			})
			...
			MiscSection:Toggle({
				...
				Flag = "HideBattleUI",
				Default = false,
				Callback = function(hideBattleUI)
					print("Hide Battle Toggled")
					if hideBattleUI == true then
						print("setting hide battle ui connection")
						connections["HideBattleUI"] = plr.PlayerGui.ScreenGui.BattleFrame
							:GetPropertyChangedSignal("Visible")
							:Connect(function()
								print("Battle start")
								local Gui = plr.PlayerGui.ScreenGui
								if Gui.BattleFrame.Visible == true then
									for _, frame in Gui.BattleFrame:GetChildren() do
										if frame:IsA("TextButton") then
											frame.Visible = false
										end
									end
									task.wait(0.1)
									Gui.RightMenu.Visible = true
									Gui.SquadButton.Visible = true
								end
							end)
					elseif hideBattleUI == false then
						print("disconnecting hide battle ui connection")
						if connections["HideBattleUI"] then
							connections["HideBattleUI"]:Disconnect()
						end
						for _, frame in plr.PlayerGui.ScreenGui.BattleFrame:GetChildren() do
							if frame:IsA("TextButton") then
								frame.Visible = true
							end
						end
			...
			MiscSection:Toggle({
				Name = "Minimize Award UI",
				Flag = "MinimizeAwardUI",
				Default = false,
				Callback = function(minimizeAwardUI)
					print("Minimize Award UI Toggled")
					if minimizeAwardUI == true then
						connections["MinimizeAwardUI"] = plr.PlayerGui.ScreenGui.ChildAdded:Connect(function(frame)
							if frame.Name == "Won" then
								if plr.PlayerGui.ScreenGui:FindFirstChild("MinimizedWonUI") then
									plr.PlayerGui.ScreenGui.MinimizedWonUI:Destroy()
								end
								local clone = frame:Clone()
								clone.Name = "MinimizedWonUI"
								clone.Parent = plr.PlayerGui.ScreenGui
								clone.Size = UDim2.fromScale(0.2, 0.3)
								clone.Position = UDim2.fromScale(0.11, 0.28)
								clone.Rotation = 0
								frame.Visible = false
							end
						end)
					elseif minimizeAwardUI == false then
						if connections["MinimizeAwardUI"] then
							connections["MinimizeAwardUI"]:Disconnect()
						end
					end
				end,
			})
		...
		local ConfigsSection = Page:Section({ Name = "Configs", Side = 2 })
		do
			local ConfigName
...
local BattlesPage = Window:Page({ Name = "Main", Icon = "108839695397679" })
local AniPixelsPage = Window:Page({ Name = "AniPixels", Icon = "108839695397679" })
local AdventuresPage = Window:Page({ Name = "Adventures", Icon = "108839695397679" })
...
do
	workspace.Battles.ChildAdded:Connect(function(battle)
		task.wait(1)
		local myBattle = getBattle(plr, plr)
		print(myBattle, battle)
		if myBattle and battle == myBattle then
			inBattle = true
			myBattle.Destroying:Connect(function()
				inBattle = false
			end)
		end
	end)
	RS.Events.BattleResult.OnClientEvent:Connect(function(player, result)
		if player.Name == plr.Name then
			Library:Notification({
				Title = "Battle Results: ",
				Description = "You " .. result,
				Duration = 3,
				Icon = "73789337996373",
				--IconColor = {
	...
	RS.Events.PVPResult.OnClientEvent:Connect(function(player, result, p3)
		if player.Name == plr.Name then
			Library:Notification({
				Title = "PVP Results: ",
				Description = "You " .. result,
	...
	local BattleSection = BattlesPage:Section({
		Name = "Battle",
		Description = "Exploits for battles",
		Icon = "123944728972740",
		Side = 1,
	})

	BattleSection:Slider({
		Name = "Speed",
		Flag = "Speed",
		Min = 1,
		Suffix = "x",
		...
		Callback = function(speed)
			local SpeedPoint = RS.State.FastForward
			SpeedPoint.Value = 1 / speed
		end,
	})

	local BattlesList = {}
	for _, battle in workspace.Campaign:GetChildren() do
		if battle:IsA("Model") and battle:FindFirstChild("Pets") then
			local enemies = ""
			for _, v in battle.Pets:GetChildren() do
				enemies = enemies .. v.Name .. ", "
			end
			table.insert(BattlesList, battle.Name .. " [" .. enemies .. "]")
		end
	end
	workspace.Campaign.ChildAdded:Connect(function(battle)
		if battle:IsA("Model") and battle:FindFirstChild("Pets") then
			local enemies = ""
			for _, v in battle.Pets:GetChildren() do
				enemies = enemies .. v.Name .. ", "
	...
	workspace.Campaign.ChildRemoved:Connect(function(battle)
		if battle:IsA("Model") and battle:FindFirstChild("Pets") then
			local enemies = ""
			for _, v in battle.Pets:GetChildren() do
				enemies = enemies .. v.Name .. ", "
			end
			table.remove(BattlesList, table.find(BattlesList, battle.Name .. " [" .. enemies .. "]"))
		end
	end)

	BattleSection:Label("List of Battles: ")
	BattleSection:Listbox({
		Flag = "BattlesList",
		Default = "",
		Size = 300,
		Items = BattlesList,
		...
		Callback = function(selected)
			selectedBattle = string.split(selected, " [")[1]
			enemies = ""
			local battle = workspace.Campaign:FindFirstChild(selectedBattle)
			if not battle then
				return
			end
			for _, v in battle.Pets:GetChildren() do
			...
			enemiesLabel:SetText("Enemies: " .. enemies)
		end,
	})
	enemiesLabel = BattleSection:Label("Enemies: " .. enemies)
	BattleSection:Toggle({
		Name = "Autofarm",
		Flag = "Autofarm",
		Default = false,
		Callback = function(Value)
			...
			while autoBattle == true do
				battle(selectedBattle)
				local battleTick = tick()
				repeat
					task.wait()
				until not inBattle or tick() - battleTick > battleFS or not autoBattle -- failsafe in case battle doesn't start for some reason
				task.wait(battleCD)
			end
		end,
	})

	local SpecificBattleSection = BattlesPage:Section({
		Name = "Functions",
		Description = "More specific stuff",
		Icon = "123944728972740",
		Side = 2,
	})

	local AUA = false
	local targetLabel = SpecificBattleSection:Label("Target: None")
	SpecificBattleSection:Toggle({
		Name = "Autofarm Unlock Areas",
		Flag = "AUA",
		Default = false,
		Callback = function(Value)
			AUA = Value
			...
			if AUA == true then
				local required = {}
				for _, area in workspace.Map.Doors:GetChildren() do
					if area:FindFirstChild("Required") then
						for __, anipixel in area.Required:GetChildren() do
							if checkForAniPixel(anipixel.Name) == false then
								table.insert(required, anipixel.Name)
								print(anipixel.Name)
							end
						end
					end
				...
				for _, anipixel in required do
					if not AUA then
						break
					end
					targetLabel:SetText("Target: " .. anipixel)
					local currentBattle = nil
					for __, battles in workspace.Campaign:GetChildren() do
						if battles:IsA("Model") and battles:FindFirstChild("Pets") then
							if battles.Pets:FindFirstChild(anipixel) then
					...
					if currentBattle then
						repeat
							if
								(
									typeof(currentBattle) == "String"
									and not workspace.Campaign[currentBattle].Pets:FindFirstChild(anipixel)
								)
								or (
									typeof(currentBattle) == "Instance"
									and not currentBattle.Pets:FindFirstChild(anipixel)
								)
							then
								for __, battles in workspace.Campaign:GetChildren() do
									if battles:IsA("Model") and battles:FindFirstChild("Pets") then
							...
							repeat
								task.wait()
							until not inBattle or tick() - battleTick > battleFS -- failsafe in case battle doesn't start for some reason
							task.wait(battleCD)
						until checkForAniPixel(anipixel) == true or not AUA
					else
						continue
					end
				end
				targetLabel:SetText("Done!")
			end
		end,
	})
end
...
do
	local currentTeam = {}
	local anipixelsList = {}
	local allAnipixels = require(RS.DoodleInfo).Info
	local autoMerge = false
	local mergeEvent = workspace.Map.Lab.Crushinator.Activate.Prompt
	local MergeSection = AniPixelsPage:Section({
		Name = "Merge",
		Description = "Quick Merge Functions!",
		Icon = "123944728972740",
		Side = 1,
	})

	local mergeList = {}
	for anipixelName, anipixelInfo in allAnipixels do
		if
			string.find(anipixelName, "Giant")
			or string.find(anipixelName, "Mega")
			or string.find(anipixelName, "Titan")
			or string.find(anipixelName, "Sparking")
		then
			continue
		end
		table.insert(anipixelsList, anipixelName)
	end
	MergeSection:Listbox({
		Flag = "MergeList",
		Default = "",
		Size = 300,
		Items = anipixelsList,
		...
		Callback = function(selected)
			for _, name in selected do
				mergeList[name] = {
					...
					[4] = "Titan " .. name,
					[5] = "Sparking " .. name,
					[6] = "Giant Sparking " .. name,
					[7] = "Mega Sparking " .. name,
				}
			end
		end,
	})
	MergeSection:Toggle({
		...
		Callback = function(Value)
			...
			if Value == true then
				currentTeam = { {} }
				for _, anipixel in plr.SavedSquad:GetChildren() do
					if anipixel:IsA("StringValue") then
						currentTeam[1][anipixel.Name] = anipixel.Value
					end
				end
			end

			while autoMerge == true do
				for merge, types in mergeList do
					print("Merging: ", merge, types)
					for index, fullName in types do
						print("Merging2: ", index, fullName)
						if not plr.DoodleInv:FindFirstChild(fullName) then
							continue
						end
						local hrp = plr.Character.HumanoidRootPart
						local disCheck = hrp.Position - workspace.Map.Lab.Crushinator.Activate.Position
						if (disCheck).Magnitude > 10 then
							repeat
								task.wait()
							until (hrp.Position - workspace.Map.Lab.Crushinator.Activate.Position).Magnitude
									< 10
								or not autoMerge
							continue
						end

						if plr.DoodleInv[fullName].Value >= 4 then
							print(fullName .. " is being merged!")
							local args = {
								{ ["1"] = fullName, ["2"] = fullName, ["3"] = fullName, ["4"] = fullName },
							}
							RS.Events.SquadUpdate:FireServer(unpack(args))
							task.wait(0.1)
							fireproximityprompt(mergeEvent, 5)

							if string.find(fullName, "Titan") then
								repeat
									task.wait()
								until plr.PlayerGui.ScreenGui.MergeSparkingConfirmation.Visible == false
							end
							print("Merged!")
							task.wait(4)
						end
			...
			print("(Merge) Equiping the original team!")
			RS.Events.SquadUpdate:FireServer(unpack(currentTeam))
		end,
	})
end
...
function checkForAniPixel(name)
	if plr.DoodleInv:FindFirstChild(name) then
		return true
	end
	return false
end

function getBattle(player, enemy)
	for _, battle in workspace.Battles:GetChildren() do
		print(battle.LeftChar.Value.Name, player.Name)
		if battle.LeftChar.Value.Name == player.Name or battle.RightChar.Value.Name == player.Name then
			print("Battle found!")
			return battle
		end
		if battle.LeftChar.Value.Name == enemy.Name or battle.RightChar.Value.Name == enemy.Name then
			return battle
		end
	end
	return nil
...
function alreadyBattling(player, enemy)
	for _, battle in ipairs(workspace.Battles:GetChildren()) do
		if battle.LeftChar.Value.Name == player.Name or battle.RightChar.Value.Name == player.Name then
			return true
		end
		if battle.LeftChar.Value.Name == enemy.Name or battle.RightChar.Value.Name == enemy.Name then
...
function battle(Battle)
	if typeof(Battle) == "string" then
		Battle = workspace.Campaign:FindFirstChild(Battle)
	end
	if not Battle then
		return
	end
	...
	local args = { Battle }
	inBattle = true
	RS.Events.RequestBattle:FireServer(unpack(args))
	-- local myBattle = nil
	-- repeat
	-- 	task.wait()
	-- 	myBattle = getBattle(plr, plr)
	-- until myBattle ~= nil or not inBattle
end
